...
#define DT_DRV_COMPAT microchip_introspect_i3c

#include <app/drivers/introspect_i3c.h>
...
static int i3c_data_get(const struct device *dev, uint8_t *val, uint32_t len)
{
...
static int i3c_data_set(const struct device *dev, uint8_t *val, uint32_t len)
{
	struct introspect_data *data = dev->data;
	int ret;
...
static int i3c_data_set_get(const struct device *dev, uint8_t *set_val, uint32_t slen,
...
#define INTROSPECT_DEFINE(inst)                                                                    \
	...
	SENSOR_DEVICE_DT_INST_DEFINE(inst, introspect_init, NULL, &introspect_data_##inst,         \
				     ...
				     CONFIG_INTROSPECT_I3C_INIT_PRIORITY, &intro_driver_api);
#else

#warning "Introspect as Master"
#define INTROSPECT_DEFINE(inst)                                                                    \
	SENSOR_DEVICE_DT_INST_DEFINE(inst, introspect_init, NULL, NULL, NULL, POST_KERNEL,         \
				     CONFIG_INTROSPECT_I3C_INIT_PRIORITY, &intro_driver_api);
#endif
